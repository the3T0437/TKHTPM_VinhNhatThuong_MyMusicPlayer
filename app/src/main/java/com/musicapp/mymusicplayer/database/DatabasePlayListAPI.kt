...
interface OnDatabasePlayListAPICallback {
    fun onSuccess(id: Long)
    fun onFailure(e: Exception)
}

class DatabasePlayListAPI(context: Context) {
    private lateinit var playListDAO: PlayListDAO

    // Khởi tạo CSDL nếu chưa tồn tại và trả về đối tượng  cho các thao tác với CSDL
    init {
        playListDAO = MyRoomDatabasePlayList.getDatabase(context).playListDao()
    }
    ...
    fun themPlayList(playList: PlayList, callback: OnDatabasePlayListAPICallback) {
        // Mở một tiến trình mới cho việc truy xuất CSDL
        coroutineScope.launch {
            try {
                val id = playListDAO.themPlayList(playList)
                // Cập nhật id cho đối tượng mới tạo trên giao diện
                if (id != -1L) {
    ...
    fun docPlayList(danhSachNhanSu:ArrayList<PlayList>, callback: OnDatabasePlayListAPICallback) {
        // Mở một tiến trình mới cho việc truy xuất CSDL
        coroutineScope.launch {
            try {
                // Doc du lieu ve tu CSDL
                val danhsachDocVe = playListDAO.docPlayList()
                // Dua toan bo  doc ve tu CSDL vao tham bien  de truyen ra Listview
    ...
    fun capNhatPlayList(playList: PlayList, callback: OnDatabasePlayListAPICallback) {
        // Mở một tiến trình mới cho việc truy xuất CSDL
        coroutineScope.launch {
            try {
